...
public class ObjectUtils {
    ...
    public static boolean isEmptyTrim(Object object) {
        ...
        if (object instanceof String) {
            String string = (String) object;
            if ("".equals(string) || "null".equals(string)||"".equals(string.trim())) {
                return true;
            }
        }
        return false;
    ...
    public static Object recursionMap(Map<String, Object> map, String getKey) {
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (val instanceof Map) {
                    recursionMap((Map<String, Object>) val, getKey);
                } else if (String.valueOf(val).startsWith("{") && String.valueOf(val).endsWith("}")) {
                    JSON jasonObject = (JSON) JSON.toJSON(val);
                    recursionMap((Map<String, Object>) jasonObject, getKey);
                }
    ...
    public static boolean isAnyEmpty(CharSequence ...charSequences){
        if (Objects.isNull(charSequences) || charSequences.length == 0) {
            return true;
        }
        return Arrays.stream(charSequences).anyMatch(ObjectUtils::isEmptyTrim);
    }
